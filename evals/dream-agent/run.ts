...
async function runCase(evalCase: (typeof dreamAgentEvalCases)[number]) {
  ...
  if (deterministicResponse) {
    return {
      evaluation: evaluateDreamAgentResult(evalCase, deterministicResponse, true),
      artifact: {
...
async function main() {
  ...
  const completed = [];
  for (let index = 0; index < filteredCases.length; index += 3) {
    const batch = filteredCases.slice(index, index + 3);
