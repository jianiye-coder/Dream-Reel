...
The candidate policy is independently canaried with `DREAM_AGENT_GUARDED_PERCENT`. It defaults to 0%, preserving `legacy-v1` after deployment while baseline telemetry accumulates; `guarded-v2` can then advance through staged percentages without conflating the policy experiment with the separate JSON response-format experiment. Immediate-crisis handling remains enabled for every user.
...
At the end of cycle 4 this was not a promotion pass: a subsequent full fresh run was interrupted by Groq's 200,000-token daily limit, and the original 20-case calibration exercise remained open. The next cycle records the reviewer's decision to replace that completion exercise with formative qualitative feedback for the current iteration.

## Cycle 5: qualitative review adaptation

The reviewer chose to stop the repetitive 20-case completion exercise and use the latest partial export as formative product feedback instead of a promotion gate. The export contained 16 case records and 10 written comments; no comment text or dream content is committed or logged. Because most dropdown scores were intentionally incomplete, this artifact is not treated as a pass, failure, or model-ranking result.

The strongest recurring issue was excessive precision: the agent still tried to identify or sharpen people and details the reviewer had already described as unclear. A second theme was insufficient emotional acknowledgment before the next question. The guarded policy now treats blur, uncertainty, and missing identity as valid dream information. It responds with explicit permission to keep the gap, avoids identity/appearance/sensory interrogation, and offers at most one broad association about atmosphere or change. This path is deterministic in Chinese and English, adds no provider latency or token cost, and is covered by a paired synthetic regression.
